...
use App\Filament\Resources\SmaQuizResource\Pages;
use App\Filament\Resources\SmaQuizResource\RelationManagers;
...
class SmaQuizResource extends Resource
{
    protected static ?string $model = SmaQuiz::class;
        ...
        return $form
            ->schema([
                Section::make('General')
                    ->schema([
                        ...
                        TextInput::make('external_id')
                            ->label('External ID')
                            ->numeric()
                            ->default(fn() => (SmaQuiz::max('external_id') ?? 0) + 1)
                            ->readOnly()
                            ->required(),
                    ]),
                ...
                Section::make('Pertanyaan')
                    ->schema([
                        Repeater::make('smaQuestions')
                            ->relationship('smaQuestions')
                            ->schema([
                                TextInput::make('name')
                                    ->label('Pertanyaan')
                                    ->required()
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('total_questions')
                    ...
                    ->getStateUsing(fn($record) => SmaQuestion::where('sma_quiz_id', $record->id)->count())
                    ->sortable(),
            ...
            ->actions([
                ...
                ->url(fn ($record) => SmaQuizResource::getUrl('hasil', ['record' => $record->id])),
            ])
            ->bulkActions([
    ...
    {
        return [
            'index' => Pages\ListSmaQuizzes::route('/'),
            'create' => Pages\CreateSmaQuiz::route('/create'),
            'edit' => Pages\EditSmaQuiz::route('/{record}/edit'),
            'hasil' => Pages\SmaResultQuiz::route('/{record}/result'),
        ];
    }
}
